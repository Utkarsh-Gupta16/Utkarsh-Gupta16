...
USER = sys.argv[1] if len(sys.argv) > 1 else "Utkarsh-Gupta16"
OUT  = sys.argv[2] if len(sys.argv) > 2 else "streak.svg"

def calc_level(count):
    if count == 0: return 0
    if count <= 5: return 1
...
def get_data(user):
    ...
    try:
        ...
        # Fallback to local data/contributions.json (generated by fetch_contributions.py)
        here = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "contributions.json")
        if os.path.exists(here):
            print("API failed (%s); mapping local data/contributions.json" % e)
            raw = json.load(open(here))
            return {
                "total": {"lastYear": raw.get("total_contributions", 0)},
                "contributions": [
                    {"date": d["date"], "count": d["count"], "level": calc_level(d["count"])} 
