...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(ROOT, "avi-ascii.svg")
CONFIG_PATH = sys.argv[3] if len(sys.argv) > 3 else os.path.join(ROOT, "profile.json")
...
try:
    ...
    print(f"Warning: {CONFIG_PATH} not found. Using default fallback config.")
    CONFIG = {
        "name": "Utkarsh Gupta",
        "username": "Utkarsh-Gupta16",
...
FULL_NAME = CONFIG.get("name", USERNAME)
...
# Improved density ramp: strictly ordered by visual weight for clean mapping
RAMP = " .`:-=+*cs#%@"

# The prepped image already has bg removed + CLAHE local contrast, 
# so only light global tuning is needed here.
CONTRAST = 1.05
BRIGHTNESS = 1.0
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(USERNAME)}@github: ~$ ./portrait.sh</text>')
...
# Dynamically generate the status bar prompt from config
cmd_str = f'{html.escape(USERNAME)}@github:~$ whoami'
name_str = html.escape(FULL_NAME)
tspan_name = f' <tspan fill="{INK}">{name_str}</tspan>'

parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{cmd_str}{tspan_name}</text>')

# Calculate precise X position for the blinking cursor based on text length
# Approximate char width at font-size 13 for the status bar text
status_text_len = len(cmd_str) + 1 + len(name_str)
cursor_x_offset = PAD + (status_text_len * 7.8) 

parts.append(f'<rect x="{cursor_x_offset:.1f}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
